...
def load_sastrawi():
    ...
    more_stop_words = [
        ...
        "nya", "kali", "aja", "nih", "sih", "deh", "dong", "kok", "yg", "ga",
        "gak", "banget", "buat", "jadi", "aplikasi", "byond", "bsi", "by",
        "mau", "buka", "masuk", "sering", "padahal", "kalo", "kalau", "bikin",
        "terus"
    ]
...
KATA_NEGATIF_SET = {
    "gagal", "susah", "lama", "error", "lambat", "lemot", "kecewa", "buruk",
    "jelek", "parah", "rugi", "blokir", "terblokir", "hilang", "potong",
    "ribet", "sulit", "kendala", "masalah", "sampah", "reset", "turun",
    "kurang", "benci", "kapok", "pindah", "bug", "hang", "stuck", "lelet",
    "payah", "rusak", "batal", "denda", "keluar", "salah", "terganggu", "malah"
}
...
def create_sentiment_pie(sentiment_count):
    ...
    # Pemetaan Warna 3 Kelas Sentimen
    color_map = {
        "Positive": "#2ecc71",
        "Positif": "#2ecc71",
        "Neutral": "#f1c40f",
...
with st.sidebar:
    ...
    st.caption("Klasifikasi 3 Kelas (Positif, Netral, Negatif)")
    st.divider()
    ...
    if uploaded_file is not None:
        try:
            ...
            if "df_result" in st.session_state:
                ...
                # Metric Cards 5 Kolom (Terdiri dari Total, Positif, Netral, Negatif, Dominan)
                st.subheader("Ringkasan Sentimen Multi-Kelas")
                sentiment_count = df["Sentimen_Prediksi"].value_counts()

                col1, col2, col3, col4, col5 = st.columns(5)
                total_ulasan = len(df)
                total_positif = sentiment_count.get("Positive", 0) + sentiment_count.get("Positif", 0)
                total_netral = sentiment_count.get("Neutral", 0) + sentiment_count.get("Netral", 0)
                total_negatif = sentiment_count.get("Negative", 0) + sentiment_count.get("Negatif", 0)
                sentimen_dominan = (
                ...
                col3.metric("🟡 Netral", total_netral)
                col4.metric("🔴 Negatif", total_negatif)
                col5.metric("Dominan", sentimen_dominan)
                ...
                with v_col2:
                    ...
                    k_tab1, k_tab2, k_tab3 = st.tabs(
                        ["🟢 Positif", "🟡 Netral", "🔴 Negatif"]
                    )

                    df_pos = df[df["Sentimen_Prediksi"].isin(["Positive", "Positif"])]
                    df_neu = df[df["Sentimen_Prediksi"].isin(["Neutral", "Netral"])]
                    df_neg = df[df["Sentimen_Prediksi"].isin(["Negative", "Negatif"])]

                    with k_tab1:
                        if not df_pos.empty:
                            top_pos = get_top_keywords(df_pos["Teks_Bersih"], top_n=8)
                            df_top_pos = pd.DataFrame(top_pos, columns=["Kata", "Frekuensi"])
                            st.dataframe(df_top_pos, hide_index=True, use_container_width=True)
                        else:
                            st.info("Tidak ada data ulasan positif.")

                    with k_tab2:
                        if not df_neu.empty:
                            top_neu = get_top_keywords(df_neu["Teks_Bersih"], top_n=8)
                            df_top_neu = pd.DataFrame(top_neu, columns=["Kata", "Frekuensi"])
                            st.dataframe(df_top_neu, hide_index=True, use_container_width=True)
                        else:
                    ...
                    with k_tab3:
                        if not df_neg.empty:
                            top_neg = get_top_negative_keywords(
                                df_neg["Teks_Bersih"], KATA_NEGATIF_SET, top_n=8
                            )
                            df_top_neg = pd.DataFrame(top_neg, columns=["Kata", "Frekuensi"])
                            st.dataframe(df_top_neg, hide_index=True, use_container_width=True)
                        else:
                ...
                with f_col1:
                    options = ["Semua"] + list(df["Sentimen_Prediksi"].unique())
...
# ==================================================
    ...
    user_input = st.text_area(
        ...
        placeholder="Contoh: Aplikasi ini biasa saja, kadang bisa dipake kadang tidak.",
        height=120,
    )
            ...
            with st.spinner("Memproses teks dan menghitung probabilitas..."):
                ...
                if model is not None and vectorizer is not None:
                    ...
                    with col_r1:
                        ...
                        elif pred in ["Neutral", "Netral"]:
                            st.warning(f"### 🟡 Sentimen: **{pred.upper()}**")
                        else:
...
# ==================================================
    ...
    st.markdown(
        """
    Aplikasi ini dibangun untuk melakukan analisis sentimen multi-kelas (**Positif, Netral, Negatif**) terhadap ulasan pengguna aplikasi **BYOND by BSI** pada platform **Google Play Store**.
    ...
    3. **Data Preparation:** Pembersihan data, pelabelan 3 kelas (Rating 1–2: Negatif, Rating 3: Netral, Rating 4–5: Positif), tokenisasi, filtering stopword Sastrawi, stemming, dan pembentukan TF-IDF vectorizer.
    4. **Modeling:** Pelatihan algoritma **Naive Bayes**, **SVM**, dan **Random Forest**.
    5. **Evaluation:** Evaluasi performa model melalui matriks Akurasi, Precision, Recall, dan F1-Score (Multi-class).
